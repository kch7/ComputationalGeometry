...
def TriangulationPlot(ax,triangles:list,points:list):
    ...
    ax.set_title("Delaunay Triangulation via Lifting Method")
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.axis('equal')
    ax.grid(True)
...
def DelaunayTriangulation(x:list,y:list,hull:list): 
    triangles=[] #In this function , I break up each face of the hull, and i convert the coordinates of each face from tuple to np.ndarray. 
    for face in hull: 
        v0=np.array(face.vertices[0],dtype=float) 
        v1=np.array(face.vertices[1],dtype=float) 
        v2=np.array(face.vertices[2],dtype=float) #Here, we compute the cross product between the differences of the vertices. 
        normal = np.cross(v1 - v0, v2 - v0) 
       
        if normal[2] < 0: 
            triangle_2d=[(v[0],v[1]) for v in face.vertices] #In the line below , i enclose the triangle by appending the first vertex to the result. 
